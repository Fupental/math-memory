...
def _build_metadata(args, memories: List[Dict], train_queries: List[Dict], test_queries: List[Dict]) -> Dict:
    return {
        ...
        "candidate_mode": "mixed_random_semantic",
        "shot_num": args.shot_num,
        "candidate_num": args.random_candidate_num + args.semantic_candidate_num,
...
def _mixed_candidates(
    ...
    random_pool = [memory_id for memory_id in range(memory_size) if memory_id not in semantic_set]
    rng = random.Random(seed + query_id * 1_000_003)
    random_ids = rng.sample(random_pool, random_candidate_num)
    candidate_ids = list(semantic_ids) + random_ids
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Generate math-memory supervision by mixed random+semantic candidate recall "
    ...
    if candidate_num > memory_size:
        raise ValueError(f"candidate_num={candidate_num} exceeds memory_size={memory_size}")
    if args.shot_num > candidate_num:
        raise ValueError("shot_num cannot exceed candidate_num")
    ...
    expected_rows_per_query = args.beam_size
    ...
    if rows:
        ...
        print(
            f"Resuming mixed generation from {len(completed_query_ids)}/{len(train_queries)} "
    ...
    query_embeddings = load_or_create_embeddings(
        str(
            cache_dir
            / f"mixed_train_queries_{safe_name}_seed{args.seed}_ratio{args.train_ratio}_anchor{len(train_queries)}.pt"
        ),
    ...
    semantic_lists = _semantic_candidates(
        query_embeddings=query_embeddings,
        memory_embeddings=memory_embeddings,
        candidate_num=args.semantic_candidate_num,
        batch_size=args.recall_batch_size,
        device=args.recall_device,
    )
    semantic_by_query_id = {
        int(query["query_id"]): candidate_ids
        for query, candidate_ids in zip(train_queries, semantic_lists)
    }
    ...
    for query in tqdm(train_queries, desc="Generating mixed D_M", ncols=100):
        query_id = int(query["query_id"])
        if query_id in completed_query_ids:
            continue
        candidate_ids = _mixed_candidates(
            query_id=query_id,
            memory_size=memory_size,
            semantic_ids=semantic_by_query_id[query_id],
            random_candidate_num=args.random_candidate_num,
            seed=args.seed,
        )
        query_rows = generate_for_query_with_candidates(
            query=query,
            memories=memories,
            scorer=scorer,
            candidate_ids=candidate_ids,
    ...
    metadata["is_complete"] = len(rows) == expected_total_rows
    _write_output_atomic(output_path, metadata, rows)
    print(f"Saved {len(rows)} mixed training sequences to {output_path}")
