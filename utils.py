...
def load_ds(cfg, split=None):
    ...
    caption 和 VQA 需要本地 COCO/VQAv2 路径；其他文本任务尝试走 HuggingFace
    datasets。返回的数据集会被 `generate_data.py`、`train.py` 和
    `icl_inference.py` 复用，确保 anchor、index、test 的字段命名一致。
    """
...
def get_info_score(
    ...
    """计算一个候选 ICD 带来的 InfoScore 增益。

    InfoScore 衡量的是加入候选 ICD 后，基础模型对正确答案 y 的条件概率提升：

    score(c) = log P(y | x, c, existing ICDs) - log P(y | x, existing ICDs)

    `choosed_icd_seq_list` 的最后一个元素是 query/anchor，前面是已经选中的 ICD。
    函数会遍历 candidate_set，把每个候选插到左侧或右侧，返回每个候选的增益。
    """

    # 1. 计算P(y|x)
    # 1.1 拼接文本输入
    kwargs = dict(add_image_token=True)
    # 避免 train.py 顶层导入 OpenICL/OpenFlamingo；这里只按类名判断纯 LLM 接口。
    if interface.__class__.__name__ == "LLMInterface":
        kwargs = dict()
    ...
    # 第一轮 beam search 时还没有任何已选 ICD，FlamingoInterface.concat_prompt
    # 需要至少一个 query 样本，不能直接传空列表。此时 mask context 只包含
    # query 的前缀部分即可，对应 InfoScore 里的 P(y|x) 基线。
    if choosed_icd_seq_list[:-1]:
...
def get_cider_score(
    ...
    """caption 任务使用 CIDEr 作为候选 ICD 的外部质量分数。

    先让 LVLM 在当前 ICD 前缀下生成 caption，得到 baseline CIDEr；再逐个加入
    候选 ICD 生成 caption，候选分数为 CIDEr 提升量。这个分数用于 beam search
    选择更适合作为 Lever-LM 监督的 ICD 序列。
    """
...
def get_lever_lm_path(cfg):
    """解析推理时要加载的 Lever-LM checkpoint。

    如果配置没有显式给 `lever_lm_path`，就到默认 checkpoint 目录下按关键字搜索。
    """
...
def init_lever_lm(cfg, lever_lm_path):
    """加载训练好的 Lever-LM 和对应 CLIP/AutoProcessor。"""
