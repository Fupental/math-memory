...
class MixSimSampler(BaseSampler):
    ...
    论文里的 candidate set 可以来自多种启发式。这里按 `sampler_ratio`
    把总候选数拆给随机、文本相似度、图像相似度三种采样器，然后拼接成每个
    anchor 的最终候选集合。这样既保留相似样本，也保留一定随机探索。
    """
    ...
    def sample(self, train_ds):
        # 三种子采样器共享 anchor_idx_list，返回结构都为 {anchor: [candidate...]}。
