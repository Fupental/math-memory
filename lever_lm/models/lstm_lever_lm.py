...
class LSTMLeverLM(BaseLeverLM):
    """基于 LSTM 的轻量 Lever-LM。

    该实现和 GPT2LeverLM 的数据语义一致：训练集样本编号就是可生成 token，
    LSTM 根据 query 特征和已选 ICD 前缀预测下一个 ICD id。区别只在底层
    序列模型更小、更简单，loss 需要在本文件里手动 shift。
    """
    ...
    def __init__(
        ...
        # index_ds 中每个样本都是一个可生成 ICD token，额外保留特殊 token。
        ...
        # adapter 将 CLIP 输出映射到 LSTM embedding 维度。
        if self._adapter:
            if "image" in need_encoder:
    ...
    def forward(self, query_input, icd_input, icd_seq_idx):
        ...
        # LSTM 没有内置 language modeling loss，因此手动做 next-token shift。
    ...
    def generation(
        ...
        和 GPT2LeverLM 相同，从 [BOS, QUERY] 开始逐步贪心生成 ICD id。
        """
        icd_input = None
        ...
        for s_n in range(shot_num):
            ...
            # 屏蔽特殊 token 和当前序列里已经选择过的 ICD，避免无效或重复示例。
            ...
            if icd_text_list or icd_img_list:
                # 扁平处理后 reshape 回每条 query 对应的 ICD 前缀。
