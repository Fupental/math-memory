...
def load_hf_ds(
    ...
    """加载 HuggingFace 数据集并补齐 idx 字段。

    Lever-LM 全链路都用样本 index 表示 ICD token，因此无论原始数据集是否自带
    id，都需要补一个连续的 `idx` 字段。
    """
...
def load_coco_ds(
    ...
    支持普通 COCO annotation，也支持 Karpathy split。输出统一包含 `image`、
    `captions`、`single_caption`、`idx` 等字段，供 scorer、Lever-LM 和推理复用。
    """
...
def load_vqav2_ds(
    ...
    """加载 VQAv2 数据集并构造图像路径、答案字段和 q_a 文本字段。"""
